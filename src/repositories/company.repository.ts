...
const CompanySchema: Schema = new Schema(
  {
    ...
    formType: { type: String, default: 'company' },
  },
  { timestamps: true, collection: 'companyenquiries' }
);

export const CompanyModel =
  mongoose.models.CompanyEnquiry ||
  mongoose.model<ICompanyDocument>('CompanyEnquiry', CompanySchema);

export interface FilterCompanyOptions {
  search?: string;
  status?: EnquiryStatus;
  area?: string;
  page?: number;
...
export class CompanyRepository {
  private inMemoryEnquiries: Map<string, CompanyEnquiry> = new Map();

  async create(data: CreateCompanyDto): Promise<CompanyEnquiry> {
    try {
      await db.connect();
      ...
      console.warn('[CompanyRepository] MongoDB not reached, saving in-memory:', err);
      const fallback: CompanyEnquiry = {
  ...
  async findAll(): Promise<CompanyEnquiry[]> {
    try {
      await db.connect();
  ...
  async findWithFilters(options: FilterCompanyOptions): Promise<{ items: CompanyEnquiry[]; total: number; page: number; totalPages: number }> {
    ...
    try {
      ...
      const docs = await CompanyModel.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit);
  ...
  async getStats(): Promise<{ total: number; pending: number; reviewed: number; archived: number }> {
    try {
      ...
      const pending = await CompanyModel.countDocuments({ status: 'PENDING' });
      const reviewed = await CompanyModel.countDocuments({ status: 'REVIEWED' });
      const archived = await CompanyModel.countDocuments({ status: 'ARCHIVED' });
  ...
  async findById(id: string): Promise<CompanyEnquiry | null> {
    try {
      await db.connect();
  ...
  async updateStatus(id: string, status: EnquiryStatus): Promise<CompanyEnquiry | null> {
    try {
      await db.connect();
      const doc = await CompanyModel.findByIdAndUpdate(id, { status }, { new: true });
      if (!doc) {
  ...
  private mapDoc(doc: ICompanyDocument): CompanyEnquiry {
    return {
      id: doc._id.toString(),
      name: doc.name,
      company: doc.company,
