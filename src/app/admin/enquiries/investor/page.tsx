...
import { InvestorEnquiry, EnquiryStatus } from '@/models/investor.model';

interface FilterResult {
  items: InvestorEnquiry[];
  total: number;
  page: number;
  totalPages: number;
}
...
function formatAreaLabel(area: string): string {
  if (!area) return 'Investor Relations';
  return area
    .split('-')
...
function InvestorBadgeIcon() {
  return (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#C6A15B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function EnquiryRow({
  ...
  item: InvestorEnquiry;
  onStatusChange: (id: string, status: EnquiryStatus) => void;
  onSelect: (item: InvestorEnquiry) => void;
...
export default function AdminInvestorEnquiriesPage() {
  ...
  const fetchEnquiries = async () => {
    ...
    const res = await apiFetch<FilterResult>(`/api/admin/enquiries/investor?${queryParams.toString()}`);
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      if (res.success && res.data && res.data.items && res.data.items.length > 0) {
        ...
        const filename = `gcc-investor-enquiries-${today}.csv`;
        ...
        setExportMessage(search || statusFilter ? 'No investor enquiries match current filters.' : 'No investor enquiries available.');
        setTimeout(() => {
          setExportStatus('idle');
          setExportMessage('');
        }, 4000);
      ...
      setExportMessage('Unable to export investor enquiries. Please try again.');
      setTimeout(() => {
        setExportStatus('idle');
        setExportMessage('');
      }, 4000);
  ...
  const handleStatusChange = async (id: string, newStatus: EnquiryStatus) => {
    const res = await apiFetch(`/api/admin/enquiries/investor/${id}`, {
      method: 'PATCH',
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this investor enquiry record?')) return;
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '30px auto', padding: '0 20px' }}>
      <div style={{ marginBottom: '25px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '3px 10px', backgroundColor: 'rgba(198, 161, 91, 0.12)', border: '1px solid rgba(198, 161, 91, 0.3)', borderRadius: '4px', color: '#C6A15B', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' }}>
            <InvestorBadgeIcon />
            <span>INVESTOR ENQUIRIES</span>
          </div>
          <h1 style={{ color: '#F4F1E9', margin: 0, fontSize: '1.75rem', fontWeight: 700, letterSpacing: '-0.01em' }}>Investor Form Submissions</h1>
          <p style={{ color: '#788692', margin: '6px 0 0 0', fontSize: '0.875rem' }}>Manage investor relations, roadshows, and meeting requests</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '15px', marginBottom: '25px', background: '#141D26', padding: '16px', borderRadius: '6px', border: '1px solid #24323F' }}>
        <input
          type="text"
          placeholder="Search investor name, organization, or email..."
          value={search}
          onChange={(e) => {
      ...
      <div style={{ background: '#141D26', border: '1px solid #24323F', borderRadius: '6px', overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#788692', fontSize: '0.875rem' }}>Loading investor enquiries...</div>
        ) : data && data.items.length > 0 ? (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              <thead>
                <tr style={{ background: '#0D141C', borderBottom: '1px solid #24323F' }}>
                  <th style={{ padding: '16px 18px', color: '#788692', fontWeight: 600, fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase' }}>INVESTOR</th>
                  <th style={{ padding: '16px 18px', color: '#788692', fontWeight: 600, fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase' }}>ORGANIZATION & CONTACT</th>
          ...
          <div style={{ color: '#788692', padding: '40px', textAlign: 'center', fontSize: '0.875rem' }}>No investor enquiries found.</div>
        )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(11, 17, 24, 0.85)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
          <div style={{ background: '#141D26', border: '1px solid #24323F', width: '100%', maxWidth: '560px', borderRadius: '6px', padding: '30px', boxShadow: '0 20px 40px -10px rgba(0, 0, 0, 0.8)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid #24323F', paddingBottom: '12px' }}>
              <h3 style={{ margin: 0, color: '#C6A15B', fontSize: '1.1rem', fontWeight: 700 }}>Investor Enquiry Details</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '0.875rem', color: '#B5BEC7' }}>
              <div><strong style={{ color: '#788692' }}>Investor Name:</strong> <span style={{ color: '#F4F1E9', fontWeight: 600 }}>{selectedEnquiry.name}</span></div>
              <div><strong style={{ color: '#788692' }}>Organization:</strong> <span style={{ color: '#F4F1E9', fontWeight: 600 }}>{selectedEnquiry.company}</span></div>
